...
import { ArrowLeft, X, Download, FileText, ImageIcon, Calendar, DollarSign, Zap, Calculator, Eye, Building, Send, Edit, Trash2 } from 'lucide-react';
...
interface RentLogDetailsPageProps {
  logId: string;
  onBack: () => void;
}

export function RentLogDetailsPage({ logId, onBack }: RentLogDetailsPageProps) {
  ...
  const log = state.rentLogs.find(l => l.id === logId);
    ...
    return (
      <div className="p-4 md:p-6">
        <div className="text-center py-12">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Rent log not found</h2>
          <p className="text-gray-600 mb-4">The rent log you're looking for doesn't exist.</p>
          <button
            onClick={onBack}
  ...
  const handleDeleteLog = () => {
    dispatch({ type: 'DELETE_RENT_LOG', payload: logId });
    showToast(`Rent log deleted successfully`, 'info');
    onBack();
  ...
  const handleEditSubmit = (logData: Omit<RentLog, 'id' | 'createdAt'>) => {
    const updatedLog = {
      ...log,
      ...logData,
    };
    dispatch({ type: 'UPDATE_RENT_LOG', payload: updatedLog });
    showToast(`Rent log updated successfully`, 'success');
    setIsEditModalOpen(false);
  };

  const generateWhatsAppMessage = () => {
    const monthYear = new Date(log.date).toLocaleDateString('en-US', { 
      month: 'long', 
      year: 'numeric' 
    });
    ...
    return `🧾 Rent received for ${monthYear} on ${day} ${month} – ${log.tenantName}

🏠 Rent: ₹${log.rentPaid.toLocaleString()}
🔌 Meter: ${log.previousMeterReading} → ${log.currentMeterReading}
⚡️ Units: ${log.units} @ ₹${log.unitPrice}/unit
💡 Meter Bill: ₹${log.meterBill.toLocaleString()}
💰 Total Received: ₹${log.total.toLocaleString()}`;
  };
  ...
  const handleSendWhatsApp = () => {
    ...
    if (tenant?.phoneNumber) {
      ...
      showToast(`WhatsApp message sent to ${log.tenantName}`, 'success');
    } else {
      // Fallback to general WhatsApp if no phone number
      const whatsappUrl = `https://wa.me/?text=${encodedMessage}`;
      window.open(whatsappUrl, '_blank');
      showToast(`WhatsApp message prepared for ${log.tenantName}`, 'info');
    }
  };
    ...
    <div className="p-4 md:p-6 pb-20 md:pb-6 md:pt-28">
      ...
      <div className="hidden md:block fixed top-0 left-64 right-0 bg-white border-b border-gray-200 z-30 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-reddit-sans-bold text-gray-900">{log.tenantName}</h1>
              <p className="text-sm font-reddit-sans-regular text-gray-600">{formatDate(log.date)}</p>
            </div>
          </div>
      ...
      <div className="md:hidden flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">{log.tenantName}</h1>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 md:p-6 border border-blue-100">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-6 mb-4 md:mb-6">
              <div className="bg-white rounded-lg p-2 md:p-4 border border-blue-200">
                ...
                <p className="text-sm md:text-lg font-semibold text-gray-900">{log.collector}</p>
              </div>
              <div className="bg-white rounded-lg p-2 md:p-4 border border-blue-200">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Date</p>
                <p className="text-sm md:text-lg font-semibold text-gray-900">{formatDate(log.date)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 md:p-6 border border-green-100">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-6">
              <div className="bg-white rounded-lg p-2 md:p-4 border border-green-200">
                ...
                <p className="text-sm md:text-lg font-semibold text-gray-900">{log.previousMeterReading}</p>
              </div>
              <div className="bg-white rounded-lg p-2 md:p-4 border border-green-200">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Current Reading</p>
                <p className="text-sm md:text-lg font-semibold text-gray-900">{log.currentMeterReading}</p>
              </div>
              <div className="bg-white rounded-lg p-2 md:p-4 border border-green-200">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Units Consumed</p>
                <p className="text-sm md:text-lg font-semibold text-gray-900">{log.units}</p>
              </div>
              <div className="bg-white rounded-lg p-2 md:p-4 border border-green-200">
                <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Unit Price</p>
                <p className="text-sm md:text-lg font-semibold text-gray-900">₹{log.unitPrice} per unit</p>
              </div>
            </div>
          </div>
          ...
          {log.notes && (
            <div className="bg-gradient-to-br from-gray-50 to-slate-50 rounded-xl p-4 md:p-6 border border-gray-200">
              ...
              <div className="bg-white rounded-lg p-3 md:p-4 border border-gray-200">
                <p className="text-sm md:text-base text-gray-700 leading-relaxed">{log.notes}</p>
              </div>
            </div>
          )}
